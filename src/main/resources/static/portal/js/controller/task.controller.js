app.controller("TaskController", function($scope, $timeout, TaskService){
    ...
    $scope.messageDelete = '';
            ...
            $scope.loadProposals = function () {
                TaskService.loadProposals().then(function(response){
                    $scope.proposals = response;
                });
            };

            $scope.deleteProposal = function(proposal){
                TaskService.deleteProposal(proposal.id).then(function(response){
                    $scope.messageAnalyze = ''
            ...
            $scope.analyzeProposal = function(proposal){
                TaskService.analyzeProposal(proposal).then(function(response){
                   $scope.loadProposals();
